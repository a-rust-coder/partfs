...
pub struct BiosParameterBlock {
    ...
    ///  - [ 0xEB, 0x??, 0x90 ]
    ///  - [ 0xE9, 0x??, 0x?? ]
    pub(in crate::filesystems::fat::fat12) jmp_boot: [u8; 3],

    /// OEM Name identifier. Can be any desired value. Part of the BS.
    pub(in crate::filesystems::fat::fat12) oem_name: [u8; 8],

    /// Count of bytes per sector. Must be one of 512, 1024, 2048, 4096.
    pub(in crate::filesystems::fat::fat12) bytes_per_sector: u16,

    /// Count of sectors per cluster. Must be a power of 2.
    pub(in crate::filesystems::fat::fat12) sectors_per_cluster: u8,
    ...
    pub(in crate::filesystems::fat::fat12) reserved_sectors_count: u16,

    /// Number of FATs on the volume. 2 is recommended.
    pub(in crate::filesystems::fat::fat12) number_of_fats: u8,
    ...
    /// `bytes_per_sector`.
    pub(in crate::filesystems::fat::fat12) root_entries_count: u16,
    ...
    pub(in crate::filesystems::fat::fat12) total_sectors_16: u16,

    /// Legal values are 0xF0, 0xF8, 0xF9, 0xFA, 0xFB, 0xFC, 0xFD, 0xFE, 0xFF.
    pub(in crate::filesystems::fat::fat12) media: u8,

    /// Count of sectors occupied by one FAT.
    pub(in crate::filesystems::fat::fat12) fat_size: u16,

    /// Irrelevant on volumes without geometry.
    pub(in crate::filesystems::fat::fat12) sectors_per_track: u16,

    /// Irrelevant on volumes without geometry.
    pub(in crate::filesystems::fat::fat12) number_of_heads: u16,

    /// Count of sectors preceding the partition that contains this FAT volume.
    pub(in crate::filesystems::fat::fat12) hidden_sectors: u32,
    ...
    pub(in crate::filesystems::fat::fat12) total_sectors_32: u32,

    /// Must be 0x00 or 0x80.
    pub(in crate::filesystems::fat::fat12) drive_number: u8,

    /// Must be 0.
    pub(in crate::filesystems::fat::fat12) reserved0: u8,

    /// BS field. Must be 0x29 if the following fields are present.
    pub(in crate::filesystems::fat::fat12) boot_signature: u8,

    /// BS field.Volume serial number. Should be generated by combining current time and date.
    pub(in crate::filesystems::fat::fat12) volume_id: u32,

    /// BS field. Volume label. Must match the 11-byte volume label recorded in the root
    /// directory. By default, should be "NO NAME    ".
    pub(in crate::filesystems::fat::fat12) volume_label: [u8; 11],

    /// BS field. Must be "FAT12   " or "FAT     ". Informational only, does not determine the
    /// FAT type.
    pub(in crate::filesystems::fat::fat12) fs_type: [u8; 8],

    /// BS field. Contains the boot code in case of bootable media.
    pub(in crate::filesystems::fat::fat12) boot_code: [u8; 448],

    /// Must be 0xAA55 (little endian)
    pub(in crate::filesystems::fat::fat12) signature: u16,
}
